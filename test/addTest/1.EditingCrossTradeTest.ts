...
describe("CrossTradeNativeTONTest", function () {
  ...
  describe("deployContract", () => {
    ...
    it("L2CrossTrade set chainInfo", async () => {
      await (await L2CrossTradeProxy.connect(l1Wallet).chainInfo(
        L1CrossTradeContract.address,
        l1ChainId
      )).wait()

      let tx = await L2CrossTradeContract.chainCrossContract(l1ChainId)
  ...
  describe("CrossTrade Test", () => {
    ...
    it("If you using providerCT enter the original fwAmount value, it will be reverted.", async () => {
      ...
      await expect(L1CrossTradeContract.connect(l1user1).provideCT(
        l2NativeToken,
        predeployedAddress.LegacyERC20ETH,
        l2Wallet.address,
        threeETH,
        ...
        200000,
        saleInformation.hashValue
      )).to.be.rejectedWith("check edit fwAmount")
    })
    ...
    it("providerCT(TON) in L1", async () => {
      ...
      const providerApproveTx = await l2NativeTokenContract.connect(l1user1).approve(L1CrossTradeContract.address, oneETH)
      await providerApproveTx.wait()
