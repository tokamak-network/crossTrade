...
describe("CrossTradeReProvideTest-Titan", function () {
    ...
    describe("CrossTrade Test", () => {
      describe("requestNonRegisteredToken & cancel", () => {
        ...
        it("cancel(TON) in L1", async () => {
          let beforel2NativeTokenBalance = await l2mockTON.balanceOf(
            l2user1.address
          )

          let beforel2NativeTokenBalanceWallet = await l2mockTON.balanceOf(
            l2Wallet.address
          )

          let beforeL2CrossTradeBalance = await l2mockTON.balanceOf(
            L2CrossTradeContract.address
          )
