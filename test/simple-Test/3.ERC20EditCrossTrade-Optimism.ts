...
describe("ERC20 CrossTrade Optimism", function () {
  ...
  describe("CrossTrade ERC20 Test", () => {
    describe("Request Test", () => {
      it("Request(Request ERC20) in L2", async () => {
        let beforel2Balance = await l2erc20Token.balanceOf(l2Wallet.address)
        let beforeL2CrossTradeBalance = await l2erc20Token.balanceOf(L2CrossTradeOP.address)
        
        const providerApproveTx = await l2erc20Token.connect(l2Wallet).approve(L2CrossTradeOP.address, threeETH)
        ...
        let afterL2CrossTradeBalance = await l2erc20Token.balanceOf(L2CrossTradeOP.address)
    ...
    describe("Edit & Provide Test", () => {
      ...
      it("editFee(ERC20) in L1", async () => {
        ...
        let beforeEditAmount = await L1CrossTradeOP.editCtAmount(saleInformation.hashValue)
        expect(beforeEditAmount).to.be.equal(0)

        const editTx = await L1CrossTradeOP.connect(l1Wallet).editFee(
          erc20Token.address,
          l2erc20Token.address,
          threeETH,
          twoETH,
        ...
        let afterEditAmount = await L1CrossTradeOP.editCtAmount(saleInformation.hashValue)
      ...
      it("approve ERC20 in L1", async () => {
        let allowance = await erc20Token.allowance(l1user1.address, L1CrossTradeOP.address)
