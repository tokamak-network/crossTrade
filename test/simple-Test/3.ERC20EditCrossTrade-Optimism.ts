  ...
  describe("CrossTrade ERC20 Test", () => {
    describe("Request Test", () => {
      it("Request(Request ERC20) in L2", async () => {
        ...
        let beforeL2CrossTradeBalance = await l2erc20Token.balanceOf(L2CrossTradeV1.address)
        ...
        let afterL2CrossTradeBalance = await l2erc20Token.balanceOf(L2CrossTradeV1.address)
    ...
    describe("Edit & Provide Test", () => {
      it("editFee(ERC20) in L1", async () => {
        ...
        let beforeEditAmount = await L1CrossTradeV1.editCtAmount(saleInformation.hashValue)
        expect(beforeEditAmount).to.be.equal(0)

        const editTx = await L1CrossTradeV1.connect(l1Wallet).editFee(
          erc20Token.address,
          l2erc20Token.address,
          threeETH,
          twoETH,
        ...
        let afterEditAmount = await L1CrossTradeV1.editCtAmount(saleInformation.hashValue)
      ...
      it("approve ERC20 in L1", async () => {
        let allowance = await erc20Token.allowance(l1user1.address, L1CrossTradeV1.address)
