  ...
  describe("CrossTrade ERC20 Test", () => {
    ...
    describe("Cancel Test", () => {
      it("Cancel(ERC20) in L1", async () => {
        let beforel2Balance = await l2erc20Token.balanceOf(l2Wallet.address)
        let beforeL2Contract = await l2erc20Token.balanceOf(L2CrossTradeOP.address)

        const saleCount = await L2CrossTradeOP.saleCount()
        let saleInformation = await L2CrossTradeOP.dealData(saleCount)

        const cancelTx = await L1CrossTradeOP.connect(l1Wallet).cancelOP(
          erc20Token.address,
          l2erc20Token.address,
          threeETH,
          twoETH,
        ...
        let afterl2Balance = await l2erc20Token.balanceOf(l2Wallet.address)
        let afterL2Contract = await l2erc20Token.balanceOf(L2CrossTradeOP.address)
